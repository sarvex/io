...
def test_avro():
    ...
    assert np.all(avro("im").to_tensor().numpy() == [100.0 + i for i in range(100)])
    ...
    dataset = tfio.IODataset.from_avro(filename, schema)
    i = 0
    for v in dataset:
        re, im = v
        assert re.numpy() == 100.0 * i
...
def test_avro_partition():
    ...
    for _ in [1, 2, 3, 11, 12, 13, 50, 51, 100]:  # capacity
        avro = tfio.IOTensor.from_avro(
            filename, schema
        )  # TODO: enable capacity=capacity
        assert np.all(avro("im").to_tensor().numpy() == [100.0 + i for i in range(100)])
        assert np.all(avro("re").to_tensor().numpy() == [100.0 * i for i in range(100)])
        for step in [1, 2, 3, 10, 11, 12, 13, 50, 51, 52, 53]:
