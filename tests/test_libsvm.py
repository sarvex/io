...
def test_basic():
    ...
    features = tf.sparse.to_dense(sparse_features, validate_indices=False)
...
def test_n_dimension():
    """test_n_dimension"""
    content = [
        ["1 1:3.4 2:0.5 4:0.231", "1 1:3.4 2:0.5 4:0.231"],
...
def test_dataset():
    ...
    dataset = tfio.experimental.IODataset.from_libsvm(libsvm_file, num_features=6)
    dataset = dataset.map(
        lambda sparse_features, labels: (
            tf.sparse.to_dense(sparse_features, validate_indices=False),
            labels,
        )
    )
    expected_f = [
        [0.0, 3.4, 0.5, 0.0, 0.231, 0.0],
        [0, 0, 2.5, np.inf, 0, 0.503],
        [0, 0.105, np.nan, 2.5, 0, 0],
    ]
    expected_l = [[1], [1], [2]]
